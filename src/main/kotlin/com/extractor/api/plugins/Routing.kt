...
fun Application.configureRouting() {
    // This log call is in the correct scope
    ...
    routing {
        ...
        get("/extract") {
            // Fixed: Use 'application.log' inside the routing block
            application.log.info("Received /extract request with URL: ${call.request.queryParameters["url"]}")

            val url = call.request.queryParameters["url"]
            if (url.isNullOrBlank()) {
                application.log.warn("/extract request failed: URL parameter is missing.")
                call.respond(HttpStatusCode.BadRequest, "URL parameter is required.")
                return@get
            }

            try {
                application.log.info("Calling ExtractorLogic for URL: $url")
                val links = ExtractorLogic.extract(url)
                if (links.isNotEmpty()) {
                    application.log.info("Successfully extracted ${links.size} links for URL: $url")
                    call.respond(links)
                } else {
                    application.log.warn("No stream links found for URL: $url")
                ...
                application.log.error("Extraction failed in /extract handler for URL: $url", e)
    ...
    // This log call is also in the correct scope
